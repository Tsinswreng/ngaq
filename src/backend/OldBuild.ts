...
type BuildConfig = typeof OldBuild.defaultConfig
export default class OldBuild {
	...
	private _config:BuildConfig = lodashMerge({}, OldBuild.defaultConfig)
	...
	run(){
		...
		const copy = OldBuild.copyFile
		const copys = OldBuild.copyFiles
		fs.mkdir(
	...
	static copyFiles(fileRelativePath:string[], originDir:string, targetDir:string){
		...
		fileRelativePath.map(e=>{
			prms.push(
				OldBuild.copyFile(e, originDir, targetDir)
			)
		})
		return prms
	}
	...
	static generateRunBat(path:string){
		...
		fs.writeFileSync(path, OldBuild.runBat ,'utf8')
	}
