import type { Belong, HistoryDbRow } from "./old_HistoryDbRow";
import type {I_GetN} from '@shared/Type'

import { mkReadN } from "../mkTsvInst";
import { HistoryTsv ,HistoryLine } from "./HistoryTsv";

export class MkReadNRow implements I_GetN<Promise<HistoryDbRow[]>>{
	...
	async GetN(n: int): Promise<HistoryDbRow[]> {
		const z = this
