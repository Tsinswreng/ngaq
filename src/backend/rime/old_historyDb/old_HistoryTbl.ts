import type { HistoryDbSrc } from "./old_HistoryDbSrc"
import type {I_perBatch, I_GetN} from '@shared/Type'
import type { I_optCheckExist } from "@backend/sqlite/sqliteUtil"

import { HistoryDbRow } from "./old_HistoryDbRow"
...
`
	...
	async insertStrm(readN:I_GetN<Promise<HistoryDbRow[]>>, opt:I_perBatch){
		let perBatch = opt?.perBatch??9999
		const z = this
		const rows = await readN.GetN(perBatch)
