...
import { WordTmd as Entity_WordTmd, WordTmd } from "@backend/old_entities/WordTmd"
import { WordTmdDbRow as WordTmdDbRow } from "@backend/old_dbRow/WordTmd"
...
export class WordTmdTable extends Abs_Table{
	...
	async addOldCreatedTable(){
		...
		const masters = await OldSqlite.meta.querySqlite_master_unsafeInt(db)
		//sqlite_master中諸表之名
		...
		for(const table of tableNames){
			const stmt = await OldSqlite.prepare(db, minIdRow_sql(table))
			const row = await OldSqlite.stmtGet<WordDbRow>(stmt)
			firstRows.push(row)
			if(row != void 0){
		...
		const tmdRows =  await OldSqlite.all(db, selectAll) as WordTmdDbRow[]
