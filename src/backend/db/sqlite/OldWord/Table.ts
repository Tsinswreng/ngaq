...
import { I_DbSrc } from "../_base_deprecated/DbSrc";
...
export class WordTable extends Abs_Table{
	...
	private static async initAddWord(db:Database, table:string, word:Word|Word[]){
		...
		const runResult:RunResult[] = await fn() //await OldSqlite.transaction(db, fn)
		return runResult
	}
	...
	static genQry_insert(table: string, word:Word){
		...
		return OldSqlite.genQry_insert(table, obj)
	}
	...
	static genQry_updateById(table: string, word:Word,id: number){
		...
		const obj = Word.toDbObj($(word))
		delete obj[c.id]; delete (obj as any)[c.table]
		return OldSqlite.genQry_updateById(table, obj, id)
	}
	...
	static async saveWords(db:Database, sws:Word[]){
		...
		for(const [table, words] of tableToWordsMap){
			...
			const pr = await OldSqlite.transaction(db, fn)
			//prms.push(pr)
			ans.push(pr)
		}
		return ans
	...
	static setWordsByIds_fn(db:Database, table:string, words:Word[], ids?:number[]){
		...
		const fn = async()=>{
			...
			for(let i = 0; i < words.length; i++){
				...
				const r = await OldSqlite.stmtRun(stmt, v)
				runResult.push(r)
			}
			return runResult
		}
		//return OldSqlite.transaction(db, fn)
		return fn
	}
	...
	static async addWordsOfSameTable_fn(db:Database, words:Word[]){
		...
		async function addWordsOfSameTable(db:Database, table:string, words:Word[]){
			...
			if(wordsToInitAdd.length !== 0){
				...
				//runResult1 = await OldSqlite.transaction(db, fn)
				runResult1 = await fn()
			}
	...
	async addWordsOfSameTable(words:Word[]){
		...
		return OldSqlite.transaction(this.dbSrc.dbRaw, fn).then((d)=>{
	...
	static async qryWordByWordShape(db:Database, table:string, wordShape:string|string[]){
		...
		if(typeof wordShape === 'string'){
			...
			const fn = await OldSqlite.fn.qryValuesInColumn_fn<WordDbRow>(db, table, cl.wordShape, wordShape)
			//let d2 = await OldSqlite.qryValuesInColumn_unsafeInt_transaction<IVocaRow>(db, table, VocaTableColumnName.wordShape, wordShape)
			const d2 = await fn()
			const r = d2
		...
		async function forOne(db:Database, table:string, wordShape:string){
			...
			const r = await OldSqlite.all<WordDbRow>(db, sql, wordShape)
	...
	async selectAllNoTblName(){
		...
		const sql = await OldSqlite.SqlGenerator.selectAllIntSafe(z.dbSrc.dbRaw, z.tableName, [])
		//console.log(sql)
		const ans:WordDbRow[] = await OldSqlite.all(db, sql)
		return ans
	}
